...
Public Class WBMS_SS
    ...
    Private Sub wbmsis_load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub tb_ok_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tb_ok.Click
        Try
            ...
            For i = 0 To cn - 1
                ...
                pperc(i) = Me.DataGridView1.Rows(i).Cells("perc").Value
                preco(i) = Me.DataGridView1.Rows(i).Cells("Reco").Value
            Next
    ...
    Private Sub tb_retrieve_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tb_retrieve.Click
        Try
            ...
            DataGridView1.Rows.Clear()
            Dim cns As Integer
            sql = " select count(itemcode) cnt from STWBMIS WHERE ticketno = " & Me.tb_searchtkt.Text
            ...
            End If
            sql = " select * from STWBMIS WHERE ticketno = " & Me.tb_searchtkt.Text _
                  & "  order by slno desc "
            dpr = New OracleDataAdapter(sql, conn)
            Dim dp As New DataSet
            dp.Clear()
            ...
            For i = 0 To cns - 1
                ...
                Me.DataGridView1.Rows(0).Cells("perc").Value = dp.Tables(0).Rows(i).Item("perc")
                Me.DataGridView1.Rows(0).Cells("Reco").Value = dp.Tables(0).Rows(i).Item("reco")
            Next
    ...
    Private Sub tb_save_Click() 'Handles tb_save.Click
        Try
            ...
            cmd.Connection = conn
            cmd.Parameters.Clear()
            cmd.CommandText = "gen_iwb_dsd.gen_wbms_isarr"
            cmd.CommandType = CommandType.StoredProcedure
            ...
            Dim pperce As OracleParameter = New OracleParameter("p9", OracleDbType.Int32)
            pperce.Direction = ParameterDirection.Input
            pperce.CollectionType = OracleCollectionType.PLSQLAssociativeArray
            pperce.Value = glbvar.pperc

            Dim pprecov As OracleParameter = New OracleParameter("p9", OracleDbType.Int32)
            pprecov.Direction = ParameterDirection.Input
            pprecov.CollectionType = OracleCollectionType.PLSQLAssociativeArray
            pprecov.Value = glbvar.preco
            ...
            cmd.Parameters.Add(pperce)
            cmd.Parameters.Add(pprecov)
    ...
    Private Sub b_print_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles b_print.Click
        Try
            ...
            If conn.State = ConnectionState.Closed Then
                conn.Open()
            End If
            sql = "select gpremarks from stwbmis where ticketno = " & tb_ticketno.Text
            ...
            Else
                ...
                cmd.Parameters.Clear()
                cmd.CommandText = "update stwbmis set gpremarks = 'X' where ticketno =" & tb_ticketno.Text
    ...
    Private Sub b_new_Click(sender As Object, e As EventArgs) Handles b_new.Click
        ...
        sql = "SELECT   NVL(MAX(WBM.TICKETNO),0)+1 TKT" _
                & "  FROM   STWBMIS WBM WHERE INOUTTYPE = 'I' "
        da = New OracleDataAdapter(sql, conn)
        Dim dstk As New DataSet
        Try
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles b_deliv.Click
        ...
        End If
        sql = "SELECT   NVL(MAX(WBM.TICKETNO),0)+1 TKT" _
                & "  FROM   STWBMIS WBM WHERE INOUTTYPE = 'O' "
        da = New OracleDataAdapter(sql, conn)
        Dim dstk As New DataSet
        Try
            da.TableMappings.Add("Table", "TKTNO")
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles b_add.Click
        Try
            If DataGridView1.Rows.Count >= 11 Then
                MsgBox("Only 11 rows are allowed, Please create new ticket")
            Else
    ...
    Private Sub b_purchase_Click(sender As Object, e As EventArgs) Handles b_purchase.Click
        ...
        Dim potype() As String = {"QN", "General PO", "QPX", "PO Against Mix Material", "QX", "Mix Material"}
        If Me.tb_sledcode.Text Like "8#####" AndAlso Not potype.Contains(cb_sap_docu_type.Text) Then
            MsgBox("PO # Mandatory - Change document type to QN")
        Else
            If tb_sap_doc.Text = "QD" Then
                ZMMPOGRPROCESSIS() 'Direct Purchase
            ElseIf tb_sap_doc.Text = "QN" Then
                ZMMGENPOPROCESS() 'Direct Purchase
            End If
        End If
    End Sub
    Private Sub ZMMGENPOPROCESS()
        ...
        Else
            ...
            Try
                ...
                'pogrir = dest.Repository.CreateFunction("Z_MM_GEN_PO_PROCESS")
                'Else
                pogrir = dest.Repository.CreateFunction("Z_MM_GEN_PO_PROCESS_WOIR")
                'End If
                ...
                ''pocst.SetValue("ZZBNAME", Me.Tb_buydesc.Text) 'Buyer Name
                ...
                grcst.SetValue("ZZBNAME", "FROM PO") 'Buyer Name
                ...
                'grcst.SetValue("ZZDATEN", CDate(Me.d_newdate.Text).Year & CDate(Me.d_newdate.Text).Month.ToString("D2") & CDate(Me.d_newdate.Text).Day.ToString("D2"))
                'grcst.SetValue("ZZDATEX", CDate(Me.d_newdate.Text).Year & CDate(Me.d_newdate.Text).Month.ToString("D2") & CDate(Me.d_newdate.Text).Day.ToString("D2"))
                ...
                cmd.Connection = conn
                cmd.Parameters.Clear()
                cmd.CommandText = "curspkg_join.get_is"
                cmd.CommandType = CommandType.StoredProcedure
                ...
                Try
                    ...
                    For a = 0 To dsmltitm.Tables("mltitm").Rows.Count - 1
                        ...
                        gpozfstru.SetValue("MATNR", dsmltitm.Tables("mltitm").Rows(a).Item("ITEMCODE").ToString()) 'Material Number
                        'gpozfstru.SetValue("WERKS", glbvar.divcd) 'Material Number
                        'If tb_werks.Text = "" Then
                        gpozfstru.SetValue("WERKS", glbvar.divcd) 'Material Number
                        'Else
                        'gpozfstru.SetValue("WERKS", Me.tb_werks.Text) 'Material Number
                        'End If
                        ...
                        End If
                        gpozfstru.SetValue("MENGE", qt)
                        'gpozfstru.SetValue("MATNR", gt) 'Quantity
                ...
                Else
                    ...
                    Else
                        MsgBox("Goods Receipt  # " & pogrir.GetValue("E_MATERIALDOCNO").ToString)
                        '                          & vbCrLf & "Invoice        # " & pogrir.GetValue("E_INVOICENO").ToString
                        Me.tb_sapdocno.Text = pogrir.GetValue("E_MATERIALDOCNO").ToString
                    End If
                    ...
                    cmd.Connection = conn
                    cmd.Parameters.Clear()
                    cmd.CommandText = "gen_iwb_dsd.gen_wbms_sap_is"
                    cmd.CommandType = CommandType.StoredProcedure
                    Try
                        If cb_crinv.Checked = True Then
    ...
    Public Sub ZMMPOGRPROCESSIS()
        ...
        Else
            ...
            Dim pogrir As IRfcFunction
            'If cb_crinv.Checked = True Then
            '    pogrir = dest.Repository.CreateFunction("Z_MM_PO_GR_PROCESS")
            'Else
            pogrir = dest.Repository.CreateFunction("Z_MM_PO_GR_WO_IR_DIR")
            'End If
            Dim pohdrin As IRfcStructure = pogrir.GetStructure("I_POHEADER")
            pohdrin.SetValue("COMP_CODE", glbvar.BUKRS)
            ...
            Try
                ...
                For a = 0 To dsmltitm.Tables("mltitm").Rows.Count - 1
                    ...
                    poitmu.SetValue("MATERIAL", dsmltitm.Tables("mltitm").Rows(a).Item("ITEMCODE").ToString())
                    poitmu.SetValue("PLANT", glbvar.divcd)
                    poitmu.SetValue("STGE_LOC", glbvar.SSLGORT)
                    poitmu.SetValue("MATL_GROUP", "01")
                    If dsmltitm.Tables("mltitm").Rows(a).Item("UOM").ToString() = "PC" Then
                        qt = Convert.ToDecimal(dsmltitm.Tables("mltitm").Rows(a).Item("NETQTY").ToString())
                    Else
    ...
    Private Sub Button1_Click_1(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        End If
        sql = "SELECT   NVL(MAX(WBM.TICKETNO),0)+1 TKT" _
                & "  FROM   STWBMIS WBM WHERE INOUTTYPE = 'E' "
        da = New OracleDataAdapter(sql, conn)
        Dim dstk As New DataSet
        Try
            da.TableMappings.Add("Table", "TKTNO")
    ...
    Private Sub CheckBox1_CheckedChanged(sender As Object, e As EventArgs) Handles ch_po.CheckedChanged
        If ch_po.Checked = True Then
            cb_sap_docu_type.Text = "General PO"
            tb_sap_doc.Text = "QN"
        Else
            cb_sap_docu_type.Text = "Direct Purchase"
            tb_sap_doc.Text = "QD"
        End If
    End Sub
    ...
    Private Sub b_spo_Click(sender As Object, e As EventArgs) Handles b_spo.Click
        'Me.Tb_asno.Text = ""
        'Me.tb_itmno.Text = ""
        g_vendor = Me.tb_sledcode.Text
        g_pono = ""
        g_itmno = ""
        fdgv.Show()
    End Sub
    Public Sub poapp()
        Me.Tb_asno.Text = g_pono
        'Me.tb_itmno.Text = g_itmno
        fdgv.Close()
    End Sub

    Private Sub Button21_Click(sender As Object, e As EventArgs) Handles b_lpo.Click
        poapp()
        Tb_asno.Text = g_pono
        'tb_itmno.Text = g_itmno
    End Sub
