...
Public Class STTRANSFER
    ...
    Private Sub cmbloading()
        ...
        End If
        cmd.Parameters.Clear()
        cmd.CommandText = "curspkg_join.fitmmst"
        cmd.CommandType = CommandType.StoredProcedure
        cmd.Parameters.Add(New OracleParameter("pcomp_code", OracleDbType.Varchar2)).Value = glbvar.cmpcd
        cmd.Parameters.Add(New OracleParameter("pdiv_code", OracleDbType.Varchar2)).Value = glbvar.grpdivcd
        cmd.Parameters.Add(New OracleParameter("io_cursor", OracleDbType.RefCursor)).Direction = ParameterDirection.Output
            ...
            ElseIf tmode = 2 Then
                ...
                Try
                    ...
                    sql = "Select INTDOCNO ,INOUTTYPE, TICKETNO, VEHICLENO, CONTAINERNO, TRANSPORTER, ACCOUNTCODE, SLEDCODE, SLEDDESC,  " _
                         ...
                         & " from stwbmibds where inouttype = 'T' and TICKETNO = " & Me.tb_ticketno.Text _
        ...
        If tmode = 1 Then
            ...
            Else
                If Me.Tb_intdocno.Text = "" Then
                    ...
                    cmd.Parameters.Add(New OracleParameter("plength", OracleDbType.Varchar2)).Value = DBNull.Value
                    cmd.Parameters.Add(New OracleParameter("pwerks", OracleDbType.Varchar2)).Value = DBNull.Value
                    Try
                        cmd.ExecuteNonQuery()
                        'Dim vint As Decimal
        ...
        ElseIf tmode = 2 Then
            ...
            Else
                ...
                Try
                    ...
                    cmd.Parameters.Add(New OracleParameter("pgprem", OracleDbType.Varchar2)).Value = DBNull.Value
                    cmd.Parameters.Add(New OracleParameter("plength", OracleDbType.Decimal)).Value = DBNull.Value
                    cmd.Parameters.Add(New OracleParameter("pwerks", OracleDbType.Varchar2)).Value = DBNull.Value
                    cmd.ExecuteNonQuery()
                    conn.Close()
    ...
    Private Sub b_print1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles b_print1.Click
        Try
            ...
            If conn.State = ConnectionState.Closed Then
                conn.Open()
            End If
            sql = "select sprinted from stwbmibds where ticketno = " & tb_ticketno.Text
            da = New OracleDataAdapter(sql, conn)
            Dim dstk As New DataSet
            Try
            ...
            Else
                If Me.cb_inouttype.Text = "T" Then
                    ...
                    ' Check if it has got multiple items.
                    cmd.Parameters.Clear()
                    cmd.CommandText = "update stwbmibds set sprinted = 'X' where ticketno =" & tb_ticketno.Text
    ...
    Private Sub b_print2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles b_print2.Click
        Try
            ...
            If conn.State = ConnectionState.Closed Then
                conn.Open()
            End If
            sql = "select gprinted from stwbmibds where ticketno = " & tb_ticketno.Text
            da = New OracleDataAdapter(sql, conn)
            Dim dstk As New DataSet
            ...
            Else
                ...
                If conn.State = ConnectionState.Closed Then
                    conn.Open()
                End If
                dsql = "select sledcode from stwbmibds where ticketno = " & tb_ticketno.Text
                ...
                If Me.cb_inouttype.Text = "T" Then
                    ...
                    ' Check if it has got multiple items.
                    cmd.Parameters.Clear()
                    cmd.CommandText = "update stwbmibds set gprinted = 'X' where ticketno =" & tb_ticketno.Text
                    cmd.CommandType = CommandType.Text
                    cmd.ExecuteNonQuery()
                    conn.Close()
                ElseIf Me.cb_inouttype.Text = "W" Then
        ...
        Else
            Try
                ...
                sql = "Select INTDOCNO ,INOUTTYPE, TICKETNO, VEHICLENO, CONTAINERNO, TRANSPORTER, ACCOUNTCODE, SLEDCODE, SLEDDESC," _
                         ...
                         & " FRINTITEMCODE,FRITEMCODE,FRITEMDESC,INTIBDSNO ,STATUS,AUART,BSART,SORDERNO,DELIVERYNO,SLNO,TRANS_CHARGE,PENALTY," _
                         & " MACHINE_CHARGE,LABOUR_CHARGE,PONO,AGMIXNO,CONSNO,CCIC,OMPRICE,OMSLEDCODE,OMSLEDDESC,VBELNS,VBELND,VBELNI,COMFLG,DOCPRINT,custtype,typecode,typecatg_pt,post_date,gpremarks,sprinted,gprinted" _
                         & " from stwbmibds where inouttype = 'T' and VEHICLENO = '" & Me.tb_sveh.Text & "'" _
                         & " and status in (1,2,3) and wtstat = 'I'"
    ...
    Private Sub b_scout_Click(sender As Object, e As EventArgs)
        ...
        End If
        sql = "SELECT   NVL(MAX(WBM.TICKETNO),0)+1 TKT" _
                & "  FROM   stwbmibds WBM WHERE INOUTTYPE = 'W' "
        da = New OracleDataAdapter(sql, conn)
        Dim dstk As New DataSet
        Try
            da.TableMappings.Add("Table", "TKTNO")
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            cmd1.CommandText = " INSERT INTO stwbmibds (" _
    ...
    Private Sub b_transfer_Click(sender As Object, e As EventArgs) Handles b_transfer.Click
        Try
            ...
            Else
                ...
                sodnbil.SetValue("ZZVEHINO", tb_vehicleno.Text)
                sodnbil.SetValue("ZZREMARKS", tb_comments.Text)
                sodnbil.SetValue("ZZDATEN", CDate(Me.tb_DATEIN.Text).Year & CDate(Me.tb_DATEIN.Text).Month.ToString("D2") & CDate(Me.tb_DATEIN.Text).Day.ToString("D2"))
                sodnbil.SetValue("ZZDATEX", CDate(Me.tb_DATEOUT.Text).Year & CDate(Me.tb_DATEOUT.Text).Month.ToString("D2") & CDate(Me.tb_DATEOUT.Text).Day.ToString("D2"))
                sodnbil.SetValue("ZZTIEN", CDate(Me.tb_TIMEIN.Text).Hour.ToString("D2") & CDate(Me.tb_TIMEIN.Text).Minute.ToString("D2") & CDate(Me.tb_TIMEIN.Text).Second.ToString("D2"))
                sodnbil.SetValue("ZZTIEX", CDate(Me.tb_TIMOUT.Text).Hour.ToString("D2") & CDate(Me.tb_TIMOUT.Text).Minute.ToString("D2") & CDate(Me.tb_TIMOUT.Text).Second.ToString("D2"))
    ...
    Private Sub b_crfillup_Click(sender As Object, e As EventArgs) Handles b_crfillup.Click
        ...
        cmd.Connection = conn
        cmd.Parameters.Clear()
        cmd.CommandText = "gen_iwb_dsd.gen_wbms_iprcf"
        cmd.CommandType = CommandType.StoredProcedure
        'Try
        ...
        Try
            ...
            cmdupd.CommandText = " update stwbmibds set CFCREATED = " & Me.tb_ticketno.Text & " where ticketno = " & Me.tb_mixtkt.Text
            cmdupdcr.CommandText = " update stwbmibds set MIXTRFTKT = " & Me.tb_mixtkt.Text & " where ticketno = " & Me.tb_ticketno.Text
    ...
    Private Sub b_cribpur_Click(sender As Object, e As EventArgs) Handles b_cribpur.Click
        ...
        End If
        sql = "SELECT   NVL(MAX(WBM.TICKETNO),0)+1 TKT" _
                & "  FROM   stwbmibds WBM WHERE INOUTTYPE = 'A'"
        da = New OracleDataAdapter(sql, conn)
        Dim dstk As New DataSet
        Try
            da.TableMappings.Add("Table", "TKTNO")
        ...
        cmd.Connection = conn
        cmd.Parameters.Clear()
        cmd.CommandText = "gen_iwb_dsd.gen_wbms_iprib"
        cmd.CommandType = CommandType.StoredProcedure
        'Try
        ...
        Try
            ...
            cmdupdcr.Connection = conn
            cmdcmt.Connection = conn
            cmdupd.CommandText = " update stwbmibds set IBTKTNO = " & Me.tb_ticketno.Text & " where ticketno = " & Me.tb_cfillup.Text
            cmdupdcr.CommandText = " update stwbmibds set CFCREATED = " & Me.tb_cfillup.Text & " where ticketno = " & Me.tb_ticketno.Text
            cmdcmt.CommandText = " commit"
            cmdupd.CommandType = CommandType.Text
            cmdupdcr.CommandType = CommandType.Text
            cmdcmt.CommandType = CommandType.Text
        ...
        If CLng(Me.tb_ticketno.Text) <> 0 Then
            ...
            Else
                If tb_sapord.Text = "" AndAlso tb_sapdocno.Text = "" AndAlso tb_sapinvno.Text = "" Then
                    ...
                    cmdupd.Connection = conn
                    cmdcmt.Connection = conn
                    cmdupd.CommandText = " update stwbmibds set VBELNS = 'Cancelled',VBELND = 'Cancelled',VBELNI = 'Cancelled' where ticketno = " & Me.tb_ticketno.Text
                    cmdcmt.CommandText = " commit"
                    cmdupd.CommandType = CommandType.Text
                    cmdcmt.CommandType = CommandType.Text
                    cmdupd.ExecuteNonQuery()
