...
Public Class WBMS_PR
            ...
            ElseIf tmode = 2 Then
                ...
                Try
                    ...
                    sql = "Select INTDOCNO ,INOUTTYPE, TICKETNO, VEHICLENO, CONTAINERNO, TRANSPORTER, ACCOUNTCODE, SLEDCODE, SLEDDESC," _
                         ...
                         & " SECONDQTY ,QTY ,DATEIN ,TIMEIN ,DATEOUT ,TIMOUT ,DEDUCTIONWT ,PACKDED,DED,PRICETON ,TOTALPRICE ,RATE,REMARKS ,IBDSNO," _
                         & " FRINTITEMCODE,FRITEMCODE,FRITEMDESC,INTIBDSNO ,STATUS,AUART,BSART,SORDERNO,DELIVERYNO,SLNO,TRANS_CHARGE,PENALTY," _
                         & " MACHINE_CHARGE,LABOUR_CHARGE,PONO,AGMIXNO,CONSNO,PRACCOUNTCODE,PRSLEDCODE,PRSLEDDESC,PRSUPPCODE,PRSUPPDESC,CCIC,OMPRICE,OMSLEDCODE,OMSLEDDESC,CFCREATED,MIXTRFTKT,IBTKTNO,VBELNS,VBELND,VBELNI,COMFLG,DOCPRINT,custtype,typecode,typecatg_pt,post_date,gpremarks,sprinted,gprinted" _
                         & " from stwbmibds_pr where TICKETNO = " & Me.tb_ticketno.Text _
                         & " and status in (1,2,3) and inouttype in ('I','O','S','W')"
                    clear_scr()
                    da = New OracleDataAdapter(sql, conn)
                    'da.TableMappings.Add("Table", "mlt")
                    Dim ds As New DataSet
                    ...
                    If ds.Tables(0).Rows.Count > 0 Then
                        ...
                        End If
                        If Not (IsDBNull(ds.Tables(0).Rows(0).Item("PRSLEDDESC"))) Then
                            Me.cb_prjsledcode.Text = ds.Tables(0).Rows(0).Item("PRSLEDDESC")
                        End If
                        If Not (IsDBNull(ds.Tables(0).Rows(0).Item("PRSLEDCODE"))) Then
                            Me.tb_prjsledesc.Text = ds.Tables(0).Rows(0).Item("PRSLEDCODE")
                        End If
                        If Not (IsDBNull(ds.Tables(0).Rows(0).Item("PRSUPPDESC"))) Then
                            Me.cb_crsldc.Text = ds.Tables(0).Rows(0).Item("PRSUPPDESC")
                        End If
                        If Not (IsDBNull(ds.Tables(0).Rows(0).Item("PRSUPPCODE"))) Then
                            Me.tb_crcode.Text = ds.Tables(0).Rows(0).Item("PRSUPPCODE")
                        End If
                        If Me.cb_inouttype.Text = "I" Then
                            If Not (IsDBNull(ds.Tables(0).Rows(0).Item("SLEDCODE"))) Then
                                sledfillup = ds.Tables(0).Rows(0).Item("SLEDCODE")
                            End If
                            If Not (IsDBNull(ds.Tables(0).Rows(0).Item("PRSLEDCODE"))) Then
                                suppfillup = ds.Tables(0).Rows(0).Item("PRSLEDCODE")
                            End If
                        ElseIf Me.cb_inouttype.Text = "O" Then
                            If Not (IsDBNull(ds.Tables(0).Rows(0).Item("PRSLEDCODE"))) Then
                                sledfillup = ds.Tables(0).Rows(0).Item("PRSLEDCODE")
                            End If
                            If Not (IsDBNull(ds.Tables(0).Rows(0).Item("PRSLEDDESC"))) Then
                                suppfillup = ds.Tables(0).Rows(0).Item("PRSLEDDESC")
                            End If
                        End If
                        If Not (IsDBNull(ds.Tables(0).Rows(0).Item("INTITEMCODE"))) Then
                            Me.Tb_intitemcode.Text = ds.Tables(0).Rows(0).Item("INTITEMCODE")
                    ...
                    If cb_inouttype.Text = "I" Then
                        ...
                        glbvar.temp_crsuppcode = Me.tb_crcode.Text
                        glbvar.temp_crsuppdesc = Me.cb_crsldc.Text
                        glbvar.temp_itemcode = Me.cb_itemcode.Text
                        glbvar.temp_itemdesc = Me.tb_itemdesc.Text
                        glbvar.temp_drcode = Me.cb_dcode.Text
                        glbvar.temp_drdesc = Me.tb_DRIVERNAM.Text
    ...
    Private Sub b_save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles b_save.Click
        Try
            Dim qa = 0, qb = 0, qc = 0, qd = 0, qe = 0, qf = 0, qtyf = 0
            'Try
            'cb_itemcode_SelectedIndexChanged(sender, e)
            ...
            If conn.State = ConnectionState.Closed Then
                conn.Open()
            End If
            sql = "SELECT   wbm.firstqty,wbm.secondqty,WBM.qty,wbm.DEDUCTIONWT,wbm.DED,wbm.PACKDED" _
                & "  FROM   STWBMIBDS_PR WBM" _
                & " WHERE WBM.TICKETNO = " & Me.tb_ticketno.Text _
                & " and status in (1,2,3)"
            dach = New OracleDataAdapter(sql, conn)
            Dim dsch As New DataSet
            dach.Fill(dsch)
            conn.Close()
            If dsch.Tables(0).Rows.Count > 0 Then
                If Not (IsDBNull(dsch.Tables(0).Rows(0).Item("QTY"))) Then
                    qa = dsch.Tables(0).Rows(0).Item("QTY")
                End If
                If Not (IsDBNull(dsch.Tables(0).Rows(0).Item("DEDUCTIONWT"))) Then
                    qb = dsch.Tables(0).Rows(0).Item("DEDUCTIONWT")
                End If
                If Not (IsDBNull(dsch.Tables(0).Rows(0).Item("DED"))) Then
                    qc = dsch.Tables(0).Rows(0).Item("DED")
                End If
                If Not (IsDBNull(dsch.Tables(0).Rows(0).Item("PACKDED"))) Then
                    qd = dsch.Tables(0).Rows(0).Item("PACKDED")
                End If
                If Not (IsDBNull(dsch.Tables(0).Rows(0).Item("firstqty"))) Then
                    qe = dsch.Tables(0).Rows(0).Item("firstqty")
                End If
                If Not (IsDBNull(dsch.Tables(0).Rows(0).Item("secondqty"))) Then
                    qf = dsch.Tables(0).Rows(0).Item("secondqty")
                End If
                qtyf = Math.Abs(CDec(qf) - CDec(qe)) - CDec(qc + qd)
            End If
            If Math.Abs(CDec(qtyf)) <> qa AndAlso qf <> 0 Then
                MsgBox("Quantity is not matching - Check Deduction and Net Weight")
                b_save_Click2()
            Else
                If tmode = 1 Then
                    ...
                    Else
                        If Me.Tb_intdocno.Text = "" Then
                            ...
                            cmd.Parameters.Add(New OracleParameter("pgprem", OracleDbType.Varchar2)).Value = rtb_gprem.Text
                            Try
                                cmd.ExecuteNonQuery()
                                'Dim vint As Decimal
                ...
                ElseIf tmode = 2 Then
                    ...
                    Else
                        ...
                        Try
                            ...
                            cmd.Parameters.Add(New OracleParameter("ppSLEDCODE", OracleDbType.Varchar2)).Value = Me.tb_prjsledesc.Text
                            cmd.Parameters.Add(New OracleParameter("ppSLEDDESC", OracleDbType.Varchar2)).Value = Me.cb_prjsledcode.Text
                            cmd.Parameters.Add(New OracleParameter("pcSLEDCODE", OracleDbType.Varchar2)).Value = Me.tb_crcode.Text
                            cmd.Parameters.Add(New OracleParameter("pcSLEDDESC", OracleDbType.Varchar2)).Value = Me.cb_crsldc.Text
                            cmd.Parameters.Add(New OracleParameter("pINTITEMCODE", OracleDbType.Int32)).Value = CInt(Me.Tb_intitemcode.Text)
                            cmd.Parameters.Add(New OracleParameter("pITEMCODE", OracleDbType.Varchar2)).Value = Me.tb_itemdesc.Text
                            cmd.Parameters.Add(New OracleParameter("pITEMDESC", OracleDbType.Varchar2)).Value = Me.cb_itemcode.Text
                            ...
                            If cb_multi.Checked = True Then
                                If itmalloc = True Then
                                    ...
                                    Try
                                        ...
                                        Dim pTICKETNO As OracleParameter = New OracleParameter(":p3", OracleDbType.Int64)
    ...
    Private Sub cb_itemcode_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cb_itemcode.SelectedIndexChanged
        Try
            ...
            If Me.cb_itemcode.SelectedIndex <> -1 Then
                ...
                If tb_sap_doc.Text = "QD" Or tb_sap_doc.Text = "QMX" Or tb_sap_doc.Text = "QPJ" Then
                    Me.tb_prlist.Text = 0
        ...
        End Try
    End Sub
    Private Sub cb_fritem_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Try
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles b_multi.Click
        Try
            If tmode = 2 Then
                ...
                glbvar.gmultival = Me.cb_multi.Checked
    ...
    Private Sub b_vehino_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles b_vehino.Click
        ...
        If conn.State = ConnectionState.Closed Then
            conn.Open()
        End If
        sql = "SELECT nvl(ticketno,0) ticketno FROM WBMLOCK_PR WHERE VEHICLENO = '" & Me.tb_sveh.Text & "'"
        Dim dalk = New OracleDataAdapter(sql, conn)
        Dim dslk As New DataSet
        dalk.Fill(dslk)
        conn.Close()
        ...
        Else
            Try
                ...
                sql = "Select INTDOCNO ,INOUTTYPE, TICKETNO, VEHICLENO, CONTAINERNO, TRANSPORTER, ACCOUNTCODE, SLEDCODE, SLEDDESC," _
                         ...
                         & " SECONDQTY ,QTY ,DATEIN ,TIMEIN ,DATEOUT ,TIMOUT ,DEDUCTIONWT ,PACKDED,DED,PRICETON ,TOTALPRICE ,RATE,REMARKS ,IBDSNO," _
                         & " FRINTITEMCODE,FRITEMCODE,FRITEMDESC,INTIBDSNO ,STATUS,AUART,BSART,SORDERNO,DELIVERYNO,SLNO,TRANS_CHARGE,PENALTY," _
                         & " MACHINE_CHARGE,LABOUR_CHARGE,PONO,AGMIXNO,CONSNO,CCIC,OMPRICE,OMSLEDCODE,OMSLEDDESC,VBELNS,VBELND,VBELNI,COMFLG,DOCPRINT,custtype,typecode,typecatg_pt,post_date,gpremarks,sprinted,gprinted" _
                         & " from stwbmibds_pr where VEHICLENO = '" & Me.tb_sveh.Text & "'" _
                         & " and status in (1,2,3) and wtstat = 'I' and inouttype in ('I','O','S','W')"
    ...
    Private Sub tb_vehicleno_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles tb_vehicleno.Validated
        ...
        Catch ex As Exception
            MsgBox(ex.Message.ToString)
        End Try
        If cb_inouttype.Text = "Incoming Goods" Then
            vsql = "SELECT   WBM.VEHICLENO" _
                & "  FROM   STWBMIBDS WBM" _
                & " WHERE WBM.VEHICLENO = '" & Me.tb_vehicleno.Text & "'" _
                & " and status in (1,2) " _
                & " and wtstat = 'I' and inouttype = 'I'"
        ElseIf cb_inouttype.Text = "Outgoing Goods" Then
            vsql = "SELECT   WBM.VEHICLENO" _
                & "  FROM   STWBMIBDS WBM" _
                & " WHERE WBM.VEHICLENO = '" & Me.tb_vehicleno.Text & "'" _
                & " and status in (1,2) " _
                & " and wtstat = 'I' and inouttype = 'O'"
        ElseIf cb_inouttype.Text = "Scale Only Tickets" Then
            vsql = "SELECT   WBM.VEHICLENO" _
                & "  FROM   STWBMIBDS WBM" _
                & " WHERE WBM.VEHICLENO = '" & Me.tb_vehicleno.Text & "'" _
                & " and status in (1,2) " _
                & " and wtstat = 'I' and inouttype = 'S'"
        Else : vsql = "SELECT   WBM.VEHICLENO" _
                & "  FROM   STWBMIBDS WBM" _
                & " WHERE WBM.VEHICLENO = 'SSAADD'" _
                & " and status in (1,2) " _
                & " and wtstat = 'I' and inouttype = 'S'"
        End If

        da = New OracleDataAdapter(vsql, conn)
        Dim dstk As New DataSet
        Try
    ...
    Private Sub b_trans_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles b_trans.Click
        ...
        If conn.State = ConnectionState.Closed Then
            conn.Open()
        End If
        sql = "SELECT nvl(ticketno,0) ticketno FROM WBMLOCK_PR WHERE intdocno = " & Me.tb_trans.Text
        Dim dalk = New OracleDataAdapter(sql, conn)
        Dim dslk As New DataSet
        dalk.Fill(dslk)
        conn.Close()
    ...
    Private Sub sl_item_driv_load()
        ...
        'End If
        'cmd.Parameters.Clear()
        'cmd.CommandText = "curspkg_join_pr.sledmstcrj"
        'cmd.CommandType = CommandType.StoredProcedure
        'cmd.Parameters.Add(New OracleParameter("pcomp_code", OracleDbType.Varchar2)).Value = glbvar.cmpcd
        'cmd.Parameters.Add(New OracleParameter("pdiv_code", OracleDbType.Varchar2)).Value = glbvar.divcd
        'cmd.Parameters.Add(New OracleParameter("io_cursor", OracleDbType.RefCursor)).Direction = ParameterDirection.Output
        ...
        '    dscsld.Clear()
        '    dacsld = New OracleDataAdapter(cmd)
        '    dacsld.TableMappings.Add("Table", "inte")
        '    dacsld.Fill(dscsld)
        '    cb_crsldc.DataSource = dscsld.Tables("inte")
        '    cb_crsldc.DisplayMember = dscsld.Tables("inte").Columns("SLEDDESC").ToString
        '    cb_crsldc.ValueMember = dscsld.Tables("inte").Columns("SLEDCODE").ToString
        '    'cb_sledcode.Tag = dssld.Tables("sled").Columns("ACCOUNTCODE").ToString()
        'Catch ex As Exception
        '    MsgBox(ex.Message)
        'End Try
        ...
        Me.tb_crcode.Text = glbvar.temp_crsuppcode
        Me.cb_crsldc.Text = glbvar.temp_crsuppdesc
    ...
    Private Sub B_PO_Click(sender As Object, e As EventArgs) Handles B_PO.Click
        ...
        If tb_sap_doc.Text = "QD" Then
            ...
            B_PO.Visible = False
        ElseIf tb_sap_doc.Text = "QPJ" Then
            'Against PO FM Z_MM_GEN_PO_PROCESS ZMMGENPOPROCESS
            'ZMMPOGRPROCESS_PR() 'Against PO Purchase
            'B_PO.Visible = False
    ...
    Private Sub ZMMGENPOPROCESS()
        ...
        Else
            ...
            Try
                ...
                '    freeze_scr()
                '    'Write an update procedure for updating the documnt numbers in stwbmibds_pr
                '    'gen_wbms_sap_U
                '    cmd.Connection = conn
                '    cmd.Parameters.Clear()
    ...
    Public Sub ZSDDIRECTCONTRACT()
        ...
        cmdc.CommandText = "curspkg_join_pr.chk_multi"
        cmdc.CommandType = CommandType.StoredProcedure
        cmdc.Parameters.Add(New OracleParameter("vtktno", OracleDbType.Long)).Value = Clng(Me.tb_ticketno.Text)
        ...
        Try
            cmdc.Connection = conn
            cmdc.Parameters.Clear()
            cmdc.CommandText = "curspkg_join_pr.get_multi"
            cmdc.CommandType = CommandType.StoredProcedure
            cmdc.Parameters.Add(New OracleParameter("vtktno", OracleDbType.Long)).Value = Clng(Me.tb_ticketno.Text)
            cmdc.Parameters.Add(New OracleParameter("io_cursor", OracleDbType.RefCursor)).Direction = ParameterDirection.Output
        ...
        Else
            ...
            Try
                ...
                dcust.SetValue("ZZTICKET", Clng(Me.tb_ticketno.Text)) ' done
                ...
                dcust.SetValue("ZZDATOUT", CDate(Me.d_newdate.Text).Year & CDate(Me.d_newdate.Text).Month.ToString("D2") & CDate(Me.d_newdate.Text).Day.ToString("D2"))
                ...
                Try
                    ...
                    If CInt(dsmlti.Tables("mlt").Rows(0).Item("cnt").ToString) > 0 Then
                        ...
                        For a = 0 To dsmltitm.Tables("mltitm").Rows.Count - 1
                            ...
                            itcuststr.SetValue("ZZDECT", CDec(dsmltitm.Tables("mltitm").Rows(a).Item("DEDUCTION").ToString()) / 1000)
                            itcuststr.SetValue("ZZPACKD", CDec(dsmltitm.Tables("mltitm").Rows(a).Item("PACKDED").ToString()) / 1000)
                    ...
                    Else
                        ...
                        pqtystr.SetValue("PICK_QTY", rqty)
                        pqtystr.SetValue("PICK_UOM", "TO")
                        pqtystr.SetValue("MATERIAL", Me.tb_itemdesc.Text)
                        pqtystr.SetValue("PLANT", glbvar.divcd)
                        pqty.Append(pqtystr)
    ...
    Public Sub ZMMMIXINMATPROCESS()
        ...
        If conn.State = ConnectionState.Closed Then
            conn.Open()
        End If
        Dim cmdmix As New OracleCommand
        Dim countmix As Integer = 0
        cmdmix.Connection = conn
        cmdmix.Parameters.Clear()
        cmdmix.CommandText = "curspkg_join_pr.get_mix"
        cmdmix.CommandType = CommandType.StoredProcedure
        cmdmix.Parameters.Add(New OracleParameter("vtktno", OracleDbType.Long)).Value = CLng(Me.tb_ticketno.Text)
        cmdmix.Parameters.Add(New OracleParameter("io_cursor", OracleDbType.RefCursor)).Direction = ParameterDirection.Output
        Dim dackmix As New OracleDataAdapter(cmdmix)
        dackmix.TableMappings.Add("Table", "mix")
        Dim dsckmix As New DataSet
        dackmix.Fill(dsckmix)
        conn.Close()
        For b = 0 To dsckmix.Tables("mix").Rows.Count - 1
            countmix = countmix + 1
        Next
        'Dim coun As String = dsmlti.Tables("mlt").Rows(0).Item("cnt").ToString
        ...
        Else
            ...
            Try
                ...
                cmd.Connection = conn
                cmd.Parameters.Clear()
                cmd.CommandText = "curspkg_join_pr.get_mix"
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.Add(New OracleParameter("vtktno", OracleDbType.Long)).Value = CLng(Me.tb_ticketno.Text)
                cmd.Parameters.Add(New OracleParameter("io_cursor", OracleDbType.RefCursor)).Direction = ParameterDirection.Output
    ...
    Private Sub ZINTERBRANCHRET()
        ...
        Else
            ...
            Try
                ...
                Dim pogrir As IRfcFunction = dest.Repository.CreateFunction("ZMM_RET_CF_IBP")
                ...
                'pogrir.SetValue("SALESDOCUMENT", Me.tb_orderno.Text)
                Dim mixtab As IRfcTable = pogrir.GetTable("T_INTERBRANCH_CONSIG")
                Dim mixstr As IRfcStructure = mixtab.Metadata.LineType.CreateStructure
                mixstr.SetValue("ZZTCNO", Me.tb_ticketno.Text)
                'mixstr.SetValue("EBELN", Me.Tb_cons_sen_branch.Text)
                ...
                Else
                    MsgBox("Return Order # " & pogrir.GetValue("SALESDOCUMENT_EX").ToString _
                              & vbCrLf & "Credit Returns # " & pogrir.GetValue("E_BILLINGDOC").ToString _
                              & vbCrLf & "Consignment Fillup # " & pogrir.GetValue("LV_SALESDOCUMENT").ToString)
                    Me.tb_sapord.Text = pogrir.GetValue("SALESDOCUMENT_EX").ToString
                    Me.tb_sapdocno.Text = pogrir.GetValue("E_BILLINGDOC").ToString
                    Me.tb_sapinvno.Text = pogrir.GetValue("LV_SALESDOCUMENT").ToString
                    freeze_scr()
                    If conn.State = ConnectionState.Closed Then
                        conn.Open()
                    End If
                    ...
                    cmd.CommandType = CommandType.StoredProcedure
                    Try
                        cmd.Parameters.Add(New OracleParameter("pVBELNS", OracleDbType.Int64)).Value = pogrir.GetValue("SALESDOCUMENT_EX").ToString
                        cmd.Parameters.Add(New OracleParameter("pVBELND", OracleDbType.Char)).Value = pogrir.GetValue("E_BILLINGDOC").ToString
                        cmd.Parameters.Add(New OracleParameter("pVBELNI", OracleDbType.Char)).Value = pogrir.GetValue("LV_SALESDOCUMENT").ToString
                        cmd.Parameters.Add(New OracleParameter("pTICKETNO", OracleDbType.Char)).Value = CLng(Me.tb_ticketno.Text)
                        cmd.ExecuteNonQuery()
                        conn.Close()
                    Catch ex As Exception
                ...
                Try
                    ...
                    If CInt(dsmlti.Tables("mlt").Rows(0).Item("cnt").ToString) > 0 Then
                        ...
                        For a = 0 To dsmltitm.Tables("mltitm").Rows.Count - 1
                            ...
                            Dim orpstru As IRfcStructure = orp.Metadata.LineType.CreateStructure
                            'Changed as per Krishna and Pravin'
                            orpstru.SetValue("PARTN_ROLE", "AG")
                            orpstru.SetValue("PARTN_NUMB", Me.tb_sledesc.Text.PadLeft(10, "0"))
                            'check if the customer is a one time customer then add the test else no need.
                            orpstru.SetValue("NAME", Me.cb_sledcode.Text)
    ...
    Public Sub ZSDCWARETURN()
        ...
        Else
            Try
                ...
                Dim pgibi As IRfcFunction = saprfcdest.Repository.CreateFunction("ZSD_CWA_RETURN")
                Dim dcust As IRfcStructure = pgibi.GetStructure("CUST_FIELDS") 'CUST_FIELDS 
                dcust.SetValue("ZZTICKET", Clng(Me.tb_ticketno.Text)) ' done
                dcust.SetValue("ZZVEHI", Me.tb_vehicleno.Text) 'done
                'dcust.SetValue("ZZVNAME", Me.tb_vehicleno.Text) 'done
                ...
                pgibi.SetValue("SALESDOCUMENT", Me.tb_orderno.Text)
                ...
                Dim ritemn As IRfcTable = pgibi.GetTable("RETURN_ITEMS_IN") 'T_DELCUST_FIELD
                Dim ritemnx As IRfcTable = pgibi.GetTable("RETURN_ITEMS_INX") 'T_DELCUST_FIELD
                Dim slines As IRfcTable = pgibi.GetTable("SCHEDULE_LINES") 'T_DELCUST_FIELD
                Dim slinesx As IRfcTable = pgibi.GetTable("SCHEDULE_LINESX") 'T_DELCUST_FIELD
                Dim pqty As IRfcTable = pgibi.GetTable("PICK_QTY") 'T_DELCUST_FIELD
                ...
                Try
                    ...
                    If CInt(dsmlti.Tables("mlt").Rows(0).Item("cnt").ToString) > 0 Then
                        ...
                        For a = 0 To dsmltitm.Tables("mltitm").Rows.Count - 1
                            ...
                            Dim ritemnstr As IRfcStructure = ritemn.Metadata.LineType.CreateStructure
                            'Dim rqty As Decimal = Convert.ToDecimal(dsmltitm.Tables("mltitm").Rows(a).Item("QTY").ToString()) / 1000
                            ritemnstr.SetValue("ITM_NUMBER", itm)
                            ritemnstr.SetValue("TARGET_QTY", rqty)
                            ritemnstr.SetValue("SALES_UNIT", "TO")
                            ritemnstr.SetValue("GROSS_WEIGHT", rqty)
                            ritemnstr.SetValue("NET_WEIGHT", rqty)
                            ritemnstr.SetValue("UNTOF_WGHT", "TO")
                            ritemnstr.SetValue("PLANT", glbvar.divcd)
                            ritemn.Append(ritemnstr)

                            Dim ritemnstrx As IRfcStructure = ritemnx.Metadata.LineType.CreateStructure
                            'Dim rqty As Decimal = Convert.ToDecimal(dsmltitm.Tables("mltitm").Rows(a).Item("QTY").ToString()) / 1000
                            ritemnstrx.SetValue("ITM_NUMBER", itm)
                            ritemnstrx.SetValue("TARGET_QTY", "X")
                            ritemnstrx.SetValue("SALES_UNIT", "X")
                            ritemnstrx.SetValue("GROSS_WEIGHT", "X")
                            ritemnstrx.SetValue("NET_WEIGHT", "X")
                            ritemnstrx.SetValue("UNTOF_WGHT", "X")
                            ritemnstrx.SetValue("PLANT", "X")
                            ritemnx.Append(ritemnstrx)

                            Dim slinesstr As IRfcStructure = slines.Metadata.LineType.CreateStructure
                            'Dim rqty As Decimal = Convert.ToDecimal(dsmltitm.Tables("mltitm").Rows(a).Item("QTY").ToString()) / 1000
                            slinesstr.SetValue("ITM_NUMBER", itm)
                            slinesstr.SetValue("SCHED_LINE", sl)
                            slinesstr.SetValue("REQ_QTY", rqty)
                            slines.Append(slinesstr)

                            Dim slinesstrx As IRfcStructure = slinesx.Metadata.LineType.CreateStructure
                            'Dim rqty As Decimal = Convert.ToDecimal(dsmltitm.Tables("mltitm").Rows(a).Item("QTY").ToString()) / 1000
                            slinesstr.SetValue("ITM_NUMBER", itm)
                            slinesstr.SetValue("SCHED_LINE", sl)
                            slinesstr.SetValue("REQ_QTY", "X")
                            slinesx.Append(slinesstrx)
                    ...
                    Else
                        ...
                        ritemnstr.SetValue("TARGET_QTY", rqty)
                        ritemnstr.SetValue("SALES_UNIT", "TO")
                        ritemnstr.SetValue("GROSS_WGHT", rqty)
                        ritemnstr.SetValue("NET_WEIGHT", rqty)
                        ritemnstr.SetValue("UNTOF_WGHT", "TO")
                        ritemnstr.SetValue("PLANT", glbvar.divcd)
                        ritemn.Append(ritemnstr)
                        ...
                        ritemnstrx.SetValue("TARGET_QTY", "X")
                        ritemnstrx.SetValue("SALES_UNIT", "X")
                        ritemnstrx.SetValue("GROSS_WGHT", "X")
                        ritemnstrx.SetValue("NET_WEIGHT", "X")
                        ritemnstrx.SetValue("UNTOF_WGHT", "X")
                        ritemnstrx.SetValue("PLANT", "X")
                        ritemnx.Append(ritemnstrx)
                        ...
                        Dim slinesstrx As IRfcStructure = slinesx.Metadata.LineType.CreateStructure
                        'Dim rqty As Decimal = Convert.ToDecimal(dsmltitm.Tables("mltitm").Rows(a).Item("QTY").ToString()) / 1000
                        slinesstrx.SetValue("ITM_NUMBER", itm)
                        slinesstrx.SetValue("SCHED_LINE", sl)
                        slinesstrx.SetValue("REQ_QTY", "X")
                        slinesx.Append(slinesstrx)
                ...
                Else
                    ...
                    ''Me.tb_sapdocno.Text = sodnbil.GetValue("E_MATERIALDOCNO").ToString
                    'Me.tb_sapinvno.Text = pgibi.GetValue("E_BILLINGDOC").ToString
                    MsgBox("Return Order # " & pgibi.GetValue("SALESDOCUMENT_EX").ToString _
                          & vbCrLf & "GR # " & pgibi.GetValue("E_DELIVERY").ToString _
                          & vbCrLf & "Invoice # " & pgibi.GetValue("E_BILLINGDOC").ToString _
                          )
                    Me.tb_sapord.Text = pgibi.GetValue("SALESDOCUMENT_EX").ToString
                    Me.tb_sapdocno.Text = pgibi.GetValue("E_DELIVERY").ToString
                    Me.tb_sapinvno.Text = pgibi.GetValue("E_BILLINGDOC").ToString
                    freeze_scr()
                    'Write an update procedure for updating the documnt numbers in stwbmibds_pr
                    cmd.Parameters.Clear()
                    ...
                    cmd.CommandType = CommandType.StoredProcedure
                    Try
                        cmd.Parameters.Add(New OracleParameter("pVBELNS", OracleDbType.Int64)).Value = pgibi.GetValue("SALESDOCUMENT_EX").ToString
                        cmd.Parameters.Add(New OracleParameter("pVBELND", OracleDbType.Char)).Value = pgibi.GetValue("E_DELIVERY").ToString
                        cmd.Parameters.Add(New OracleParameter("pVBELNI", OracleDbType.Char)).Value = pgibi.GetValue("E_BILLINGDOC").ToString
                        cmd.Parameters.Add(New OracleParameter("pTICKETNO", OracleDbType.Char)).Value = Clng(Me.tb_ticketno.Text)
                        cmd.ExecuteNonQuery()
                        conn.Close()
    ...
    Private Sub b_scout_Click(sender As Object, e As EventArgs) Handles b_scout.Click
        unfreeze_scr()
        clear_scr()
        Me.tb_DATEIN.Text = Today.Date
        Me.tb_DATEOUT.Text = Today.Date
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            cmd1.CommandText = " INSERT INTO stwbmibds_pr (" _
