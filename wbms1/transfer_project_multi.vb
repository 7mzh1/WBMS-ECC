...
Public Class TRANSFER_PR_multi
            ...
            ElseIf tmode = 2 Then
                ...
                Try
                    ...
                    ElseIf Me.tb_sap_doc.Text = "QIX" Then
                        Me.Tb_cons_sen_branch.Visible = True
                    ElseIf Me.tb_sap_doc.Text = "" Then
    ...
    Private Sub b_crfillup_Click(sender As Object, e As EventArgs) Handles b_crfillup.Click
        Dim cmdck As New OracleCommand
        If conn.State = ConnectionState.Closed Then
            conn.Open()
        End If
        cmdck.Connection = conn
        cmdck.Parameters.Clear()
        cmdck.CommandText = "curspkg_join_pr.get_multi"
        cmdck.CommandType = CommandType.StoredProcedure
        cmdck.Parameters.Add(New OracleParameter("vtktno", OracleDbType.Long)).Value = CLng(Me.tb_ticketno.Text)
        cmdck.Parameters.Add(New OracleParameter("io_cursor", OracleDbType.RefCursor)).Direction = ParameterDirection.Output
        Dim damultitm As New OracleDataAdapter(cmdck)
        damultitm.TableMappings.Add("Table", "mltitm")
        Dim dsmltitm As New DataSet
        damultitm.Fill(dsmltitm)
        conn.Close()
        ...
        For a = 0 To dsmltitm.Tables("mltitm").Rows.Count - 1
            ...
            End If
            cmd.Parameters.Add(New OracleParameter("pINTITEMCODE", OracleDbType.Int32)).Value = CInt(Me.Tb_intitemcode.Text)
            cmd.Parameters.Add(New OracleParameter("pITEMCODE", OracleDbType.Varchar2)).Value = dsmltitm.Tables("mltitm").Rows(a).Item("ITEMCODE").ToString()
            cmd.Parameters.Add(New OracleParameter("pITEMDESC", OracleDbType.Varchar2)).Value = dsmltitm.Tables("mltitm").Rows(a).Item("ITEMDESC").ToString()
            Me.tb_itemdesc.Text = dsmltitm.Tables("mltitm").Rows(a).Item("ITEMCODE").ToString()
            Me.cb_itemcode.Text = dsmltitm.Tables("mltitm").Rows(a).Item("ITEMDESC").ToString()
            If Me.cb_inouttype.Text <> "T" Then
                cmd.Parameters.Add(New OracleParameter("pFRINTITEM", OracleDbType.Int32)).Value = CInt("141325")
                cmd.Parameters.Add(New OracleParameter("pFRITEM", OracleDbType.Varchar2)).Value = "Dummy"
                cmd.Parameters.Add(New OracleParameter("pFRITEMDESC", OracleDbType.Varchar2)).Value = "00000"
            ...
            cmd.Parameters.Add(New OracleParameter("pNATIONALITY", OracleDbType.Varchar2)).Value = DBNull.Value
            cmd.Parameters.Add(New OracleParameter("pDRIVINGLICNO", OracleDbType.Varchar2)).Value = DBNull.Value
            Dim firstwtd As Decimal = Convert.ToDecimal(dsmltitm.Tables("mltitm").Rows(a).Item("FIRSTQTY").ToString())
            cmd.Parameters.Add(New OracleParameter("pFIRSTQTY", OracleDbType.Decimal)).Value = firstwtd
            Me.tb_FIRSTQTY.Text = Math.Round(firstwtd)
            Dim secwtd As Decimal = Convert.ToDecimal(dsmltitm.Tables("mltitm").Rows(a).Item("SECONDQTY").ToString())
            cmd.Parameters.Add(New OracleParameter("pSECONDQTY", OracleDbType.Decimal)).Value = secwtd
            Me.tb_SECONDQTY.Text = Math.Round(secwtd)
            ...
            Else
                cmd.Parameters.Add(New OracleParameter("pDED", OracleDbType.Decimal)).Value = 0.0
            End If
            Dim qty As Decimal = Convert.ToDecimal(dsmltitm.Tables("mltitm").Rows(a).Item("QTY").ToString())
            cmd.Parameters.Add(New OracleParameter("pQTY", OracleDbType.Decimal)).Value = qty
            Me.tb_QTY.Text = Math.Round(qty)
            Dim dtin As Date = FormatDateTime(Me.tb_DATEIN.Text, DateFormat.GeneralDate)
            cmd.Parameters.Add(New OracleParameter("pDATEIN", OracleDbType.Date)).Value = dtin 'Convert.ToDateTime(Me.tb_DATEIN.Text)
            cmd.Parameters.Add(New OracleParameter("pTIMEIN", OracleDbType.Varchar2)).Value = Me.tb_TIMEIN.Text
